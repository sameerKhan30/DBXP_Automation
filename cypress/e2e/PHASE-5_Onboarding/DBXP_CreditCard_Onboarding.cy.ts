...
describe("Credit Card Onboarding", () => {
    before("Read Data From Fixture File", () => {
        ...
        // });
        cy.fixture("CreditCard_Onboarding.json").then((setData) => {
            this.setOnboardData = setData;
        });
    });

    beforeEach("Login", () => {
        cy.visit("http://dbxpinnom.comviva.com/mbw/#/mbw/login")
        // cy.visit("http://dbxp.int.comviva.com/mbw/#/mbw/login");  // Standalone Env
        cy.wait(500);
    });

    it("Credit Card Onboarding", () => {
        // Click on  Don't have an account ? button
        cy.get('[class="col-12 activation-card"]').click();

        // Click on Loan Onboarding options
        cy.contains(" Credit Card ").should("be.visible").click();
        ...
        // Validate Steps
        cy.wait(500);
        cy.contains(" Select Credit Card ").should("be.visible");
        ...
        cy.get('[class="account-creation-section"]').then((page) => {
            ...
            cy.get('[role="option"]').each((creditCardType) => {
                const types = creditCardType.text().trim();
                cy.log("Credit Card Type: " + types);

                if (types.includes("Silver")) {
                    cy.wrap(creditCardType).click();
                    cy.wait(500);
                    cy.get('[class="btn send-otp"]').click()
                }
            });
                ...
                .each((employementDropDownValues) => {
                    ...
                    if (employementTypeText.includes(chooseEmployementType)) {
                        ...
                        if (employementTypeText.includes("Retired")) {
                            ...
                            cy.wrap(page).find('[data-placeholder="Employer Name"]').type("HCL");

                            // Selecting Currency Type
                            cy.get('#mat-select-value-7').then((currencyDropdown) => {
                                cy.wrap(currencyDropdown).click()
